...
// EV, ICE, HYBRID: ACCELERATOR (0x35), ACCELERATOR_BRAKE_ALT (0x100),
// ACCELERATOR_ALT (0x105)
#define HYUNDAI_CANFD_COMMON_RX_CHECKS(pt_bus)                                 \
  {.msg = {{0x35, (pt_bus), 32, 100U, .max_counter = 0xffU,                    \
            .ignore_quality_flag = true},                                      \
           {0x100, (pt_bus), 32, 100U, .max_counter = 0xffU,                   \
            .ignore_quality_flag = true},                                      \
           {0x105, (pt_bus), 32, 100U, .max_counter = 0xffU,                   \
            .ignore_quality_flag = true}}},                                    \
      {.msg = {{0x175, (pt_bus), 24, 50U, .max_counter = 0xffU,                \
                .ignore_quality_flag = true},                                  \
               {0},                                                            \
               {0}}},                                                          \
      {.msg = {{0xa0, (pt_bus), 24, 100U, .max_counter = 0xffU,                \
                .ignore_quality_flag = true},                                  \
               {0},                                                            \
               {0}}},                                                          \
      {.msg = {{0xea, (pt_bus), 24, 100U, .max_counter = 0xffU,                \
                .ignore_quality_flag = true},                                  \
               {0},                                                            \
               {0}}},
...
#define HYUNDAI_CANFD_STD_BUTTONS_RX_CHECKS(pt_bus)                            \
  HYUNDAI_CANFD_COMMON_RX_CHECKS(pt_bus){                                      \
      .msg = {{0x1cf, (pt_bus), 8, 50U, .ignore_checksum = true,               \
               .max_counter = 0xfU, .ignore_quality_flag = true},              \
              {0},                                                             \
              {0}}},

#define HYUNDAI_CANFD_ALT_BUTTONS_RX_CHECKS(pt_bus)                            \
  HYUNDAI_CANFD_COMMON_RX_CHECKS(pt_bus){                                      \
      .msg = {{0x1aa, (pt_bus), 16, 50U, .ignore_checksum = true,              \
               .max_counter = 0xffU, .ignore_quality_flag = true},             \
              {0},                                                             \
              {0}}},

// SCC_CONTROL (from ADAS unit or camera)
#define HYUNDAI_CANFD_SCC_ADDR_CHECK(scc_bus)                                  \
  {.msg = {{0x1a0, (scc_bus), 32, 50U, .max_counter = 0xffU,                   \
            .ignore_quality_flag = true},                                      \
           {0},                                                                \
           {0}}},
...
static bool hyundai_canfd_tx_hook(const CANPacket_t *msg) {
  ...
  if (msg->addr == 0x1cfU) {
    ...
    bool allowed =
        (is_cancel && cruise_engaged_prev) || (is_resume && controls_allowed);
    if (!allowed) {
      tx = false;
    }
  }
...
static safety_config hyundai_canfd_init(uint16_t param) {
  ...
  static const CanMsg HYUNDAI_CANFD_LKA_STEERING_STD_ALT_BUTTONS_TX_MSGS[] = {
      ...
      {0x2A4, 0, 24, .check_relay = true}, /* CAM_0x2A4 */
      HYUNDAI_CANFD_SCC_CONTROL_COMMON_TX_MSGS(1, false)};
...
#define HYUNDAI_CANFD_LFA_STEERING_CAMERA_SCC_TX_MSGS(longitudinal)            \
  ...
  if (hyundai_longitudinal) {
    ...
    if (hyundai_canfd_lka_steering) {
      ...
      // EV4: custom RX checks - bypass all checksum/counter validation
      static RxCheck hyundai_canfd_lka_steering_alt_buttons_rx_checks[] = {
          ...
          {.msg = {{0x1aa, 1, 16, 50U, .ignore_checksum = true,
                   ...
                   {0},
                   {0}}},
          {.msg = {{0x1a0, 1, 32, 50U, .ignore_checksum = true,
                    .ignore_counter = true, .ignore_quality_flag = true},
                   {0},
                   {0}}},
      };
      ...
      if (hyundai_canfd_lka_steering_alt) {
        ...
        if (hyundai_canfd_alt_buttons) {
          SET_TX_MSGS(HYUNDAI_CANFD_LKA_STEERING_STD_ALT_BUTTONS_TX_MSGS, ret);
        } else {
      ...
      // Both button messages exist on some platforms, so we ensure we track the
      // correct one using flag
